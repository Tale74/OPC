...
| PODSETNIK | Current URNA/PEPEO business contract is locked and the recovery implementation includes +3 activation, existing `deliveryTimes`, completion cancellation, exact presentation/fallback, Windows in-app secondary due delivery and the scoped `ZAVRŠEN` blocker. Historical task names are provenance only. | Source/test correction is bounded; Windows/Android runtime acceptance, release and publication remain separate gates. Current database schema is 31; canonical DB is not changed by this state record. |
