...
- The URNA/PEPEO business contract is locked: relevant `KREMACIJA`/
  `KREMACIJA_EKSPRES` obligations exclude `NAKNADNO`, use the ceremony date
  plus three days and existing `deliveryTimes`, and stop when completed.
  Implementation, runtime acceptance, release and publication status remain
  separate evidence states. Windows uses the existing in-app PODSETNIK path;
  Android uses its platform notification path.
...
| PODSETNIK | Reminder/notification operational area; current URNA/PEPEO contract is locked and other signal semantics remain separately scoped |
