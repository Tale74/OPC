...
### 5C. Current URNA/PEPEO correction boundary

The URNA/PEPEO business contract is locked. Source/tests cover the scoped
secondary cycle, exact user-facing wording and fallback, the Windows in-app
secondary due path, and the `ZAVRŠEN` blocker. This is technical source/test
status only; it is not Windows/Android owner runtime acceptance, release
acceptance or publication. The current application schema is 31 and no
canonical database or migration repair is implied by this correction.
