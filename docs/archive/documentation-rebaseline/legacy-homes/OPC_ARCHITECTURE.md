...
- PODSETNIK presentation and reminder implementation are separated. The
  recovered owner-authority contract remains the business boundary; the
  current URNA/PEPEO presentation, secondary cycle and lifecycle blocker are
  implemented in the recovery worktree. Windows secondary delivery follows
  the existing in-app PODSETNIK resume/startup path, while Android retains OS
  notification delivery. Implementation, runtime acceptance, release and
  publication remain separate states; historical task names are provenance
  only and cannot create new business semantics.
...
- bounded structured PODSETNIK obligation/signal foundation and the current
  URNA/PEPEO correction are implemented; Windows/Android runtime acceptance,
  release and publication remain separate gates. The current schema is 31;
  the additive secondary notification-ID persistence is not a migration-repair
  claim.
