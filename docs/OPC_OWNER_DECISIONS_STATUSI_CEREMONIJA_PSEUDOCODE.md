...
### 5.10 IRIU confirmed dependencies — `OPC-OD-CER-010`

```text
OWNER_CONFIRMED_CURRENT_GROUPING:
  SAHRANA_VAN_SRBIJE -> [MANDATORY MEDJUNARODNI_PREVOZ, MANDATORY MEDJUNARODNA_DOKUMENTACIJA, CONDITIONAL_REMOVABLE BALSAMOVANJE]
  DOCEK_POSMRTNIH_OSTATAKA -> [CARGO_TROSKOVI]
  DOCEK_PARAMETERS -> [MESTO_DOCEKA, DATUM_DOCEKA, VREME_DOCEKA]
  BALSAMOVANJE DOES_NOT_DEPEND_ON DOCEK_POSMRTNIH_OSTATAKA
  disabled_source_rows remain stored_but_suppressed_or_inactive
  OPELO_NE suppresses stored KOMPLET_ZA_OPELO without deleting manual edits

CLASSIFY CURRENTLY_IMPLEMENTED
