...
## Current sanity-check — 2026-08-02

The post-zero Phase 4 tree is technically stable under the latest automated
review: `flutter analyze --no-pub` is clean and the complete suite reports
304 passed, 1 skipped and 0 failed. No build was run in this review. The full
evidence, anti-drift rules and remaining dependency sequence are recorded in
`docs/tasks/OPC_TASK_SANITY_CHECK_AUTOREVIEW_20260802_REPORT.md`.

This technical PASS does not authorize scenario repository/JSON materialization,
stale-row reconciliation, SCENARIO UI or PODSETNIK signal work. Those remain in
the dependency order documented below and in the authoritative plan.
...
| PREDMET lifecycle/referential design | RI-1 inventory and RI-2 hard-delete are Git-closed. Owner-approved 3A/4A full restore has historical cumulative analyze/test/build PASS. The owner later reported scoped Android full-restore runtime PASS on the INC-003 correction artifact: the unsafe reminder-section error did not recur, PREDMETI/PARTE loaded, and observed completed data/history remained intact; other data and security coverage was not tested. INC-003 correction filters reminder/history export by the captured PREDMET set, safely skips only fully valid parentless rows in already-produced schema-8 backups, prevents reused-ID reassociation, scoped-cancels all destination IDs and preserves 3A/4A. Supplied-backup preflight proves reminders 9→7, history 230→224, active trigger 0, three future platform slots rebuilt, and clean referential state. FK enforcement remains off. | Repeat cumulative owner runtime on the current schema-23 tip before release closure. Anonymization/replacement remain closed at owner gates 1/2; broad RI-3 recovery remains separate. |
