...
| Phase 1 architecture | Complete source review supports retaining the current codebase with progressive PREDMET/lifecycle/core-v2 refactor. Full rewrite is not supported. | Architecture Decision Gate waits for Windows concurrency/startup, Android PARTE profiling, SCENARIO/IRiU, parity/product-profile and reliable complete-test evidence. |
...
| Windows startup | Source audit maps pre-`runApp` window-manager/locale waits, first-query DB open, repeated steady-state recovery/validation and seed/backfill/KATALOG scans, followed by post-login status/reminder/setup work. A synthetic Flutter harness did not reach its first SQLite phase within nine minutes and produced no OPC metric. | Do not repeat the harness. Quantitative current-HEAD baseline requires separately owner-authorized isolated WINDOWS_TEST instrumentation and build; implementation remains closed. |
| Static/test validation snapshot | `flutter analyze --no-pub` is PASS on the Phase 1 review branch. The complete test command did not return a final result within the 15-minute output-wrapper limit and left an active tester child. | Test status is NOT COMPLETED, not PASS or FAIL; build gate remains closed. |
