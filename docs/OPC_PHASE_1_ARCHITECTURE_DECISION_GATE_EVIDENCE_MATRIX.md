...
| Windows startup | INSTALLED OWNER BASELINE CONFIRMED / CURRENT-HEAD BASELINE PENDING | installed owner version reaches login in approximately 8–9 s; startup changed canonical DB by +8,192 bytes while integrity remained `ok`; source maps pre-`runApp` waits, first DB query, repeated recovery/validation, 74 seed attempts, policy update and full KATALOG scan | owner-authorized current-HEAD isolated WINDOWS_TEST instrumentation/build, cold/warm phase timings and owner target | Not authorized |
