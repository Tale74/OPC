...
| SCENARIO/IRiU | COMPLETE SOURCE AUDIT / BOUNDED PARTIAL BOUNDARY REWRITE JUSTIFIED | one scenario ID over hard-coded condition families; confirmed ordering incident; owner-locked automatic reconciliation conflicts with current suppress/dialog behavior; no row provenance; no FIRMA template storage; urn-place semantic gap | owner fixture for exact `JAVNO MESTO/NEDEFINISANO` expected result and confirmation whether existing `groblje` is the same business fact as urn `MESTO CEREMONIJE`; implementation only in a new Projects chat after Architecture Gate | Not authorized |
