...
| --- | --- | --- | --- | --- | --- | --- | --- |
| TAR-001 | Business `verzija` increment matrix | Initial code-first matrix complete; SCENARIO, IRiU/contact omissions and lifecycle ambiguity confirmed. | Canonical aggregate design, migration proof and focused regression/runtime parity evidence. | Version is central to conflict/review reasoning. | Version drift or false conflict context. | YES. | `OPC_PREDMET_BUSINESS_VERSION_MATRIX_AUDIT.md` |
| TAR-002 | Change-log overview and retention | Code-first lifecycle/retention closure defines separate privacy-safe checkpoints and local audit events, minimum taxonomy, anonymization/delete retention and legacy handling. | Separate authorized implementation plus migration/recovery/privacy/version/parity proof. | Change-log could become business audit authority. | Data duplication, version drift or misleading review history. | NO — technical design complete. | `OPC_PREDMET_LIFECYCLE_LOG_RETENTION_AND_EVENT_TAXONOMY_AUDIT.md` |
| TAR-003 | Firm-scoped identity and local-user transfer | Corrected firm audit plus dedicated user-identity audit confirm the existing PREDMET-to-local-user-to-FIRMA ownership and exact new-import/replacement rebinding model. | Implementation, focused tests, full-backup preflight, collision handling and runtime parity proof. | Preserves existing ownership without importing foreign local IDs or inventing parallel identity. | Wrong local adviser attribution, unsafe restore, or duplicate case number. | YES. | `OPC_FIRM_SCOPED_PREDMET_IDENTITY_TECHNICAL_AUDIT.md`; `OPC_LOCAL_USER_IDENTITY_AND_PREDMET_TRANSFER_REBIND_AUDIT.md` |
...
| IBI-002 | Firm-scoped identity guard | Code-first identity and user-rebinding design are complete; implementation plus backup/collision test/runtime proof remain. | `IMPLEMENTATION AUTHORIZATION`; `TEST EVIDENCE`; `RUNTIME EVIDENCE` | Wrong local adviser attribution, unsafe restore, or duplicate case number. | `OPC_LOCAL_USER_IDENTITY_AND_PREDMET_TRANSFER_REBIND_AUDIT.md` |
...
| IBI-005 | Version/change-log overview changes | Business policy and technical design are complete; runtime event/checkpoint model and review list are not implemented. | `IMPLEMENTATION AUTHORIZATION`; `MIGRATION PROOF`; `TEST EVIDENCE`; `RUNTIME EVIDENCE` | Misleading business audit surface. | `OPC_PREDMET_LIFECYCLE_LOG_RETENTION_AND_EVENT_TAXONOMY_AUDIT.md`; `OPC_PREDMET_IDENTITY_VERSION_CHANGELOG_GAP_REGISTER.md` |
...
- [x] Initial technical audit complete: business `verzija` increment matrix and aggregate coverage defects characterized.
...
- [x] Bounded implementation and full QA evidence complete: local-user rebinding, four-state full-backup PIB/MB preflight, collision-safe `brojPredmeta` and the Case-2 selective fallback for new/unambiguous PREDMET families; destructive restore remains fail-closed and release/runtime parity remain separate.
