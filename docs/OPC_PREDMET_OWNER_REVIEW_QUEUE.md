...
| TAR-003 | Firm-scoped identity guard | Corrected code-first audit confirms existing PREDMET-to-local-user-to-FIRMA ownership; source-local user IDs are not portable; minute-level number collision and absent backup preflight are source-confirmed. | Local-user rebinding, collision, backup-preflight implementation/test/runtime proof. | Preserves existing ownership without inventing a parallel FIRMA identity concept. | Wrong local adviser attribution, unsafe restore, or duplicate case number. | YES. | `OPC_FIRM_SCOPED_PREDMET_IDENTITY_TECHNICAL_AUDIT.md` |
...
| IBI-002 | Firm-scoped identity guard | Corrected code-first audit is complete; local-user rebinding plus backup/collision test/runtime proof remain. | `TECHNICAL DESIGN`; `TEST EVIDENCE`; `RUNTIME EVIDENCE` | Wrong local adviser attribution, unsafe restore, or duplicate case number. | `OPC_FIRM_SCOPED_PREDMET_IDENTITY_TECHNICAL_AUDIT.md` |
