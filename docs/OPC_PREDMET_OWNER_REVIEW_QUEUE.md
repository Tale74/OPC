...
| ODR-002 | PREDMET/FIRMA identity legacy and history policy | Firm-scoped identity policy is closed and the code-first audit is complete. | Legacy single-PREDMET JSON without FIRMA identity; PIB/MB correction versus new FIRMA identity. | Completes migration compatibility without reopening the approved identity scope. | False same-firm assertion or unnecessary rejection of valid legacy data. | YES. | `OPC_FIRM_SCOPED_PREDMET_IDENTITY_TECHNICAL_AUDIT.md` section 7 |
...
| TAR-002 | Change-log overview and retention | Code-first audit completed; current log mixes raw checkpoints with audit events and owner requires local log retention across replacement. | Checkpoint/hash migration proof, exact event model, legacy-row handling, tests and UI parity. | Change-log could become business audit authority. | Data duplication, version drift or misleading review history. | YES. | `OPC_LOGIZMENA_TECHNICAL_AUDIT.md` |
| TAR-003 | Firm-scoped identity guard | Code-first audit completed; minute-level number collision, absent FIRMA JSON scope, missing history and absent backup preflight are source-confirmed. | Two owner decisions, migration fixtures, implementation/test/runtime proof. | Current conflict key is narrower than future-safe identity. | Wrong-firm transfer/restore/sync collision. | YES. | `OPC_FIRM_SCOPED_PREDMET_IDENTITY_TECHNICAL_AUDIT.md` |
...
| IBI-002 | Firm-scoped identity guard | Code-first audit is complete; two owner decisions plus migration/test/runtime proof remain. | `OWNER DECISION`; `MIGRATION PROOF`; `TEST EVIDENCE`; `RUNTIME EVIDENCE` | Wrong-firm conflict/restore/sync. | `OPC_FIRM_SCOPED_PREDMET_IDENTITY_TECHNICAL_AUDIT.md` |
...
- [x] Code-first technical audit complete: firm identity/history source and targeted architecture.
...
- [ ] Implementation remains blocked until two OWNER DECISIONS / MIGRATION PROOF / TEST EVIDENCE / RUNTIME EVIDENCE: firm-scoped identity guard.
