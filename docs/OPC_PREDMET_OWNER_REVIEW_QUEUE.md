...
| TAR-002 | Change-log overview and retention | Current review shows status/version/saved state; logs exist for snapshots/cycles; owner requires local log retention across replacement. | Suitability, snapshot privacy/retention, local import event design and UI parity. | Change-log could become business audit authority. | Incomplete or misleading review history. | YES. | `OPC_PREDMET_IDENTITY_VERSION_CHANGELOG_GAP_REGISTER.md` `OPC-PREDMET-LIV-GAP-003` and `OPC-PREDMET-LIV-GAP-004` |
...
- [ ] Technical audit required: change-log overview, snapshot privacy/retention, local import event design and replacement-log preservation.
