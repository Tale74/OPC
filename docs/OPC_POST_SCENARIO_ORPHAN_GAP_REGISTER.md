...
- The PREDMET replacement audit-history successor is now closed by the bounded `logIzmena` preservation/`IMPORT_REPLACE` transaction and focused rollback evidence; no audit-history orphan or successor remains.
- The PREDMET local-identity/recovery correction has one current authority home in `docs/OPC_PREDMET_LOCAL_IDENTITY_RECOVERY_ACCEPTANCE_REPORT.md`; full QA is PASS. Case 2 remains explicitly fail-closed with exactly one successor, `FULL-BACKUP MISSING/INCOMPLETE FIRMA IDENTITY — USER FALLBACK + SAFE MERGE/RECONCILIATION DESIGN AND ACCEPTANCE`; Case 3 fresh recovery is supported. No orphan or successor-less outcome is introduced.
