...
- The PREDMET local-identity/recovery authority now includes the Case-2 selective fallback acceptance report. Destructive replacement remains fail-closed for incomplete FIRMA identity; explicit user confirmation can import only new/unambiguous PREDMET families with local actor rebinding. Case 1 and Case 3 remain supported, and the former Case-2 successor is closed without an orphan.
