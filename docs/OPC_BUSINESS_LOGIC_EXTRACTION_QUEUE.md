...
| Logos business-critical pseudocode layer | Public pseudocode now maps business-critical source behavior for PREDMET lifecycle, evaluator, IRiU, STANJE ROBE, finance, display, PDF, JSON, backup, entitlement, auth, and review/advisor areas. | EXTRACTED PUBLIC BASELINE | Use pseudocode as learning/owner-review aid only; source code remains implementation truth and future changes still require characterization tests and gates. |
