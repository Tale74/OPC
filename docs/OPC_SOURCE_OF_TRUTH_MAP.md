...
| F-09 FINANSIJE owner clarification — 2026-09-19 | `docs/OPC_PRODUCT_AND_DOMAIN.md` §6 and `docs/OPC_OWNER_DECISION_GUIDE.md` §9.6 | Owner-provided clarification; `docs/OPC_CURRENT_DEVELOPMENT_STATE.md`; focused current-source evidence | OWNER-AUTHORIZED TARGET — FINANSIJE parent with two sibling children, exact triggers and manual/REVIEW BAR semantics; current source lacks both; broader unspecified FINANSIJE reminder scope remains deferred. No child sequencing or roadmap-member selection is created. |
