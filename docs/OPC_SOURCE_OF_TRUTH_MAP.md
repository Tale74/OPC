...
| Bounded PARTE workflow/performance and stable selector correction — 2026-09-24 | `docs/OPC_PRODUCT_AND_DOMAIN.md` §1 and §7 (PARTE remains a PREDMET-derived derivative); no new business authority | `docs/OPC_ARCHITECTURE.md` §6, current development state, `lib/features/predmeti/parte/**`, `test/parte_module_screen_test.dart`, `test/parte_print_preparation_pdf_completion_test.dart`, ČITULJE/JSON regressions | PUBLISHED / OWNER RUNTIME ACCEPTED — composer local edits avoid redundant source-change read/recomposition; completion requires a valid current plan plus confirmed preview, not prior PDF export; PARTE uses a stable-ID PREDMET dropdown with retained-preparation eligibility preserved. PDF/DOCX, PARTE→ČITULJE handoff, schema, Single-PREDMET JSON and Backup behavior remain bounded/unchanged. The source-proven correction is redundant refresh/recomposition; ČITULJE was not confirmed as the direct cause. Focused 39/39, analyzer, full 586 PASS / 10 SKIP / 0 FAIL and Windows/Android release builds pass; OWNER responsiveness acceptance passed after correction. |
| DVE VALUTE preparation closure and finalization deferral — 2026-09-25 | `docs/OPC_PRODUCT_AND_DOMAIN.md` §6 and current DVE VALUTE closure marker | `docs/OPC_CURRENT_DEVELOPMENT_STATE.md`, `docs/OPC_AUTHORITATIVE_DEPENDENCY_BASED_DEVELOPMENT_PLAN.md`, completed source-learning/design review package outside SOURCE | CURRENT ROADMAP AUTHORITY — source-learning, authority reconciliation and implementation design are complete; product implementation is deferred to the final OPC v1.5 finalization step. The accepted model preserves one durable currency per PREDMET, moves DVE VALUTE to the finalization tail, targets finalized product version 2.0, records future SRB distinction and keeps OPC Int as a later SRB-based product-line evolution. Open design choices remain deferred and do not create current product behavior. |
