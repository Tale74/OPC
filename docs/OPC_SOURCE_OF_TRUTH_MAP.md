...
| Post-drift recovery-phase governance | `docs/OPC_POST_DRIFT_RECOVERY_PLAN.md` | Accepted recovery handoffs, current development state, dependency plan, protected residual ledger and matrix | CURRENT GOVERNANCE CONTROL — canonical continuity/ordering reference during post-drift recovery; references detailed residual/deferred/evidence authority and does not create product requirements or replace the existing controls. |
