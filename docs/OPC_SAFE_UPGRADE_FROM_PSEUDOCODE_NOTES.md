...
Windows previously derived a 2/1/0-day in-app banner and Android had no scheduler -> a shared 48-hour/frequency model now cancels/replaces local IDs, skips past times, drives session-deduped dialogs, and schedules Android-only local notifications -> reminder text includes `vrstaCeremonije`, PREMINULO LICE `ime + prezime`, `datumCeremonije`, `vremeCeremonije`, and `DOVRŠITE NEOPHODNE PRIPREME` as owner-required PREDMET identity -> stale/duplicate alarms or incomplete identity can cause operational harm -> keep PREDMET facts authoritative and reminder persistence local/non-JSON; introduce no remote push/backend or implementation-invented privacy filtering.
