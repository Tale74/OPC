...
| `verzija` import comparator or hard-block behavior | Owner decision says `verzija` may inform future conflict UI, but missing/malformed, same-version, and higher/lower-version behavior are unresolved and keep/replace/cancel user choice must remain unless a later owner decision authorizes a hard block. | Version semantics design, JSON safety, repository identity, data ownership, and platform parity gates. |
