...
void main() {
  test(
    'CVEĆE ribbon text is row-scoped, transferable and current-event based',
    () async {
      final db = createTestDatabase();
      addTearDown(db.close);
      final predmetId = await db
          .into(db.predmeti)
          .insert(const PredmetiCompanion(brojPredmeta: Value('T2-RIBBON')));
      final rowA = await db
          .into(db.iriu)
          .insert(
            IriuCompanion(
      ...
      final changedB = await (db.select(
        db.iriu,
      )..where((row) => row.id.equals(rowB))).getSingle();
      ...
      final currentReadded = await (db.select(
        db.iriu,
      )..where((row) => row.id.equals(readded))).getSingle();
      expect(currentReadded.tekstTrake, equals(null));
    },
  );

  test(
    'NALOG CVEĆARI prepared data uses current IRiU order and row values',
    () async {
      final db = createTestDatabase();
      addTearDown(db.close);
      final predmetId = await db
  ...
  test('IRiU ribbon UI contract is row-scoped in the current source', () {
    ...
    expect(ribbonSource, contains("'iriu-cvece-ribbon-\${row.id}'"));
    expect(ribbonSource, contains('row.nazivPrikaz'));
    expect(ribbonSource, contains('row.tekstTrake ??'));
    expect(ribbonSource, contains('IriuCompanion(tekstTrake: Value(value))'));
  ...
  testWidgets('real assistance callbacks do not complete atomic obligations', (
    ...
    final predmetId = await db
        .into(db.predmeti)
        .insert(const PredmetiCompanion(brojPredmeta: Value('T2-ACTIONS')));
