...
const _kSpecifikacijaTitle = 'SPECIFIKACIJA TROŠKOVA';
const _kSpecifikacijaFilenameToken = 'SPECIFIKACIJA_TROSKOVA';
...
Future<void> izvoziSpecifikacijaTroskovaPdf({
  ...
  try {
    ...
    ScaffoldMessenger.of(ctx).showSnackBar(
      SnackBar(
        content: Text('SPECIFIKACIJA TROŠKOVA sačuvana: $lokacija'),
    ...
    prikaziPdfExportSuccessSnackBar(
      ctx,
      poruka: 'SPECIFIKACIJA TRO\u0160KOVA sa\u010duvana: $lokacija',
      fajl: fajl,
    );
  } catch (e) {
    if (!ctx.mounted) return;
    ScaffoldMessenger.of(ctx).showSnackBar(
      SnackBar(
        content: Text('Greška pri izvozu SPECIFIKACIJE TROŠKOVA: $e'),
        duration: const Duration(seconds: 5),
...
Future<Uint8List> _buildSpecifikacijaPdf({
...
class _SpecifikacijaPdfSnapshot {
  const _SpecifikacijaPdfSnapshot({
    required this.firma,
    required this.app,
  ...
  factory _SpecifikacijaPdfSnapshot.fromPreparedData(
...
pw.Widget _buildHeader(_SpecifikacijaPdfSnapshot snapshot) {
          ...
          children: [
            pw.Expanded(
              child: pw.Column(
                ...
                children: [
                  pw.Text(
                    documentTextCodec.normalize(_kSpecifikacijaTitle),
                    style: pw.TextStyle(
                      fontWeight: pw.FontWeight.bold,
                      fontSize: 11,
                    ),
            ...
            pw.Text(
              documentTextCodec.normalize(
                'Datum: ${formatDateForDocument(snapshot.datumDokumenta)}',
              ),
